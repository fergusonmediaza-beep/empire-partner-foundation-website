...
function initCounters() {
    const counters = document.querySelectorAll('.portfolio-metric[data-target], .stat-number[data-target]');
...
function getErrorEl(form) {
    let el = form.querySelector('.newsletter-error');
    if (!el) {
...
function clearInlineError(form) {
    ...
    const el = form.querySelector('.newsletter-error');
    if (el) {
...
function initNewsletterForm() {
    ...
    form.addEventListener('submit', function (e) {
        ...
        if (!email || atPos < 1 || atPos === email.length - 1 || email.indexOf('.', atPos) < atPos + 2) {
            showInlineError(form, 'Please enter a valid email address (e.g. name@example.com).');
            return;
        }
        ...
        .then(function (res) {
            if (res.ok) {
                ...
                showInlineError(form, 'Subscription failed. Please try again.');
            }
        })
        .catch(function () {
            showInlineError(form, 'Connection error. Please try again.');
        })
        .finally(function () {
            btn.disabled = false;
